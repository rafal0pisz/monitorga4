...
export default function ProjectConfigForm({ project }: Props) {
    ...
    try {
      ...
      if (ceRes.data) {
        ...
        if (events.length > 0 && project.ga4_property_id) {
          ga4Fetch(`/api/ga4/events?projectId=${encodeURIComponent(project.id)}&events=${encodeURIComponent(events.map((e: CustomEvent) => e.event_name).join(','))}&periodDays=14`)
            .then(r => r.json())
            .then(data => {
  ...
  useEffect(() => {
    ...
    ga4Fetch(`/api/ga4/custom-dimensions?projectId=${encodeURIComponent(project.id)}`)
      .then(async r => {
  ...
  async function openSuggestions() {
    ...
    try {
      const res = await ga4Fetch(`/api/ga4/discover-events?projectId=${encodeURIComponent(project.id)}`)
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (ceErr) throw new Error(ceErr.message.includes('ITEM_LIMIT_REACHED') ? "Your plan allows up to 5 custom events per project. Upgrade to add more." : `Custom events: ${ceErr.message}`)

      const { error: ecErr } = await supabase.rpc('save_ecommerce_config', {
        p_project_id: pid,
        p_events: [...ecomEnabled].map(event_name => ({ event_name })),
      })
      if (ecErr) throw new Error(ecErr.message.includes('ITEM_LIMIT_REACHED') ? "Your plan allows up to 5 e-commerce events per project. Upgrade to add more." : `Ecommerce: ${ecErr.message}`)
      ...
      if (pcErr) throw new Error(pcErr.message.includes('ITEM_LIMIT_REACHED') ? "Your plan allows up to 5 parameter checks per project. Upgrade to add more." : `Parameters: ${pcErr.message}`)
